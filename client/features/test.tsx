...
interface CustomTooltipProps {
  ...
  payload?: any[];
  label?: string;
  activeLine?: string | null; // Tooltip 컴포넌트에는 직접 전달되지 않지만, 로직 구성을 위해 정의
}
...
export function SectorAnalysisSection() {
  ...
  const handleTickerChange = (ticker: string) => {
    setSelections((prev) => {
      const newTickers = prev.tickers.includes(ticker)
  ...
  const handleAnalyze = useCallback(async () => {
    if (selections.tickers.length === 0) {
      setError('분석할 섹터를 1개 이상 선택해주세요.');
      return;
    }
      ...
      <CardContent className="space-y-8">
        <div className="space-y-6 p-6 border rounded-lg bg-muted/30">
          ...
          <div className="space-y-2">
            ...
            <Card className="min-h-[8rem] max-h-48 overflow-y-auto">
                ...
                <LineChart
                  ...
                  onMouseMove={(state) => {
                    if (state.isTooltipActive && state.activePayload) {
                      setActiveLine(state.activePayload[0].dataKey);
                    } else {
                      setActiveLine(null);
                    }
                  }}
